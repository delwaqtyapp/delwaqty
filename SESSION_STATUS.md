...
> **Last updated:** 2026-09-23 — **ROUND 29: «الأكثر طلبا» modern redesign — gradient-branded cards + Stories-style progress bars** — User asked for a more modern look for the auto-moving cards in the Most-Requested carousel on /market. Redesigned (commerce_discovery_page.dart only, both `_MostRequestedCard` + `_AnimatedMerchantCarousel`, ADR-044/075-safe: gradients only, NO Blur, RepaintBoundary around PageView): card shell = radius 24 + type-color LinearGradient background + 1px type-color border + colored glow shadow; left 118px image block with cover image (cacheWidth dpr×240) + bottom scrim gradient + emoji watermark + open badge (successLight capsule, top-left) + verified check circle (top-right); right text block = rating capsule (star + score + optional count) + w700 name ellipsis + Wrap of service capsules (type label / delivery minutes / fee) — zero overflow by design; indicators replaced the dot row with **story-style RTL progress bars** (3px tracks, active fills right→left via AnimationController(4s) synced with the auto-scroll, past pages stay full). 937/937 tests (existing 3 regression tests green), analyze 0 (fixed 4 const lints), APK `releases/delwaqty_1.0.0+1_debug_20260923_0150.apk` installed + relaunched clean (pid 24531). Committed sprint 169.
