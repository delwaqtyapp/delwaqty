...
> **Last updated:** 2026-09-23 — **ROUND 31: FIX — PageController.page assertion crash on «الأكثر طلبا» (sprint 170 regression)** — User hit `Failed assertion: 'positions.isNotEmpty': PageController.page cannot be accessed before a PageView is built with it` on /market. Root cause: sprint-170 depth transform computed the live page in `build()` as `_controller.page ?? _controller.initialPage.toDouble()` — `.page` THROWS from flutter's `page_view.dart:176` when the controller has no attached position yet (first frame / before attach), the `??` never got a chance. Fix (commerce_discovery_page.dart ~line 369, surgical 5-line change): guard before reading the live page — `(_controller.hasClients && _controller.position.haveDimensions) ? (_controller.page ?? _current.toDouble()) : _current.toDouble()` — falls back to the already-tracked `_current` int until the position is attached and dimensioned, preserving smooth cover-flow scaling during swipes once attached. Gates: analyze **0 issues**, tests **937/937**, APK `releases/delwaqty_1.0.0+1_debug_20260923_0305.apk` installed + relaunched clean (pid 21005, no assertion/page_view/FATAL in logcat). Committed sprint 171.
