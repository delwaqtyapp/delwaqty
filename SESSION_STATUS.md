...
> **Last updated:** 2026-09-23 — **ROUND 33 PROPORTION FIX: hero 480-540px, scaled-down content (sprint 173)** — User: hero far too tall (0.62*height ≈ huge on tall phones); wants compact premium header ≈500-540 max, foreground scaled down, welcome closer to دلوقتي, no rebuild, same assets. Surgical numeric edits (home_page.dart only): heroH `(h*0.42).clamp(480,540)`; menu+notification buttons 40/50→54 (icon 24); logo 95→80 (errorBuilder 80/80 icon 28); دلوقتي Cairo 17→15 (≈40-50px wide, intro treatment untouched); Egypt title 24→26 w900 gold, tagline 13→14; greeting subtitle 13→14, ConstrainedBox 84→72 (welcome total ~60-70); gaps: after controls 12→10, Spacer before greeting flex 3→2 (welcome rises closer to دلوقتي), bottom 14→10; pill 44 & search 54 unchanged (targets 44-48 / 52-56). Overflow check: fixed sum ≈427 ≤ 432 available at min hero (480−inset 48) → zero overflow. Verified: analyze 0, tests 937/937, `rg -c home_egypt_hero`=1, no transforms, APK `releases/delwaqty_1.0.0+1_debug_20260923_1000.apk` installed + clean relaunch (pid 20487); REAL screencap saved `releases/screenshot_hero_1000.png` (1362581 bytes) for user-side visual comparison — orchestrator has NO image input, cannot judge pixels; user compares against spec (hero ~500-540, logo ~75-85, welcome ~60-70, location 44-48, search 52-56, no overflow/overlap/white strip).
