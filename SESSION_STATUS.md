...
> **Last updated:** 2026-09-23 — **ROUND 28: white VOID between filter buttons and «الأكثر طلبا» — root cause PROVEN (Horizontal viewport was given unbounded height)** — User reported a remaining white void exactly between the filter buttons under search and the «الأكثر طلبا» section. Live-DB check: featured merchants EXIST (6+ is_featured=true) so the section has data — the void was inside the featured carousel itself. Empirical probe test reproduced the exact failure: a horizontal `ListView.separated` with NO explicit height inside the outer vertical ListView (unbounded cross axis) throws **«Horizontal viewport was given unbounded height»** — so the sprint-166 change (removing `height:180` from the featured carousel) was the DIRECT cause of the void: the ListView fails to lay out → the region renders blank → next to the crash/fatal escalation in the debug APK. Fixed: (1) wrapped the featured `ListView.separated` in `SizedBox(height: 250)` (card natural height ≈ 238 at scale 1.0; 250 gives slack); loading spinner box 180 → 250 for consistency. (2) RESTORED `Flexible` around MerchantCard's text block — now LEGAL in every host (featured carousel is bounded by the SizedBox, grid cells are bounded) and guarantees no overflow at any font scale (rows ellipsis). (3) Regression tests rewritten to the REAL structures: (a) featured carousel with SizedBox(250) renders 2 cards with NO exception; (b) a raw horizontal ListView WITHOUT bounded height must throw (documents the bug); (c) MerchantCard in a 0.74 grid cell — no overflow. Gate: `flutter analyze` **0 issues**, `flutter test` **937/937**, APK `releases/delwaqty_1.0.0+1_debug_20260923_0130.apk` installed + relaunched clean (pid 21574, no unbounded/RenderFlex/FATAL). Committed sprint 168.
