...
> **Last updated:** 2026-09-23 — **ROUND 33 FINAL v2: 5 clear vertical Hero zones (sprint 173)** — User: hero too short (saw "BOTTOM OVERFLOWED BY 42 PIXELS"); rebuild ONLY hero layout on clearly separated responsive zones, 540-600 logical px, no random widget moves, no negative margins/hard-coded Positioned, no shrinking everything. Rebuilt `_EgyptHero` (home_page.dart only) as: `heroH = (h*0.62).clamp(540,600)`; foreground = SafeArea(top,bottom:false)+Padding+Column with Spacers between zones: **ZONE1** `_buildTopControlsRow` (Directionality(ltr) forced + spaceBetween → Notification ALWAYS left, Menu ALWAYS right, both 40px glass) → gap12 → **ZONE2** `Align(topRight)` Egypt statement (gold مصر 24 w900 + white tagline 13 w600, compact, separated from menu) → Spacer(2) → **ZONE3** Brand lockup: logo `SizedBox(95×95)` contain + circular glow (no fill), wordmark 16 kept between logo and دلوقتي, دلوقتي intro-exact (Cairo w600 F7F7FA letterSpacing 0.5, rtl Directionality) fontSize 17 (~45-55% of 95 logo width) → Spacer(3) → **ZONE4** Greeting in `ConstrainedBox(maxHeight:84)` (name 24 w800 white maxLines1 ellipsis + subtitle 13 white .85 maxLines2 ellipsis) → Spacer(2) → **ZONE5** Location pill 44 (Expanded+ellipsis) → gap10 → Search 54/28 → gap14. Fixed content ≈450 ≤ available at min hero (540−inset 48=492) → mathematically zero overflow; Spacers absorb slack, zones never overlap. Verified: analyze 0, tests 937/937, `rg -c home_egypt_hero`=1, one SafeArea (hero), one `delwaqty_logo_mark` use, no flip transforms, APK `releases/delwaqty_1.0.0+1_debug_20260923_0900.apk` installed + clean relaunch (pid 4687). Visual checks user-side; drawer/menu from previous round intact.
