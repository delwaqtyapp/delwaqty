...
final supabaseReviewDataSourceProvider = Provider<SupabaseReviewDataSource>((ref) {
...
class SupabaseReviewDataSource {
  ...
  Review _fromRow(Map<String, dynamic> row) => Review(
    ...
    imageUrls: row['image_urls'] != null ? List<String>.from(row['image_urls'] as List) : [],
  ...
  Future<List<Review>> getMerchantReviews(String merchantId) async {
    final data = await _client.from('reviews').select().eq('merchant_id', merchantId).order('created_at', ascending: false);
    return (data as List).map((r) => _fromRow(r as Map<String, dynamic>)).toList();
  }

  Future<List<Review>> getProductReviews(String productId) async {
    final data = await _client.from('reviews').select().eq('product_id', productId).order('created_at', ascending: false);
    return (data as List).map((r) => _fromRow(r as Map<String, dynamic>)).toList();
  }

  Future<Review?> getReviewById(String id) async {
    final data = await _client.from('reviews').select().eq('id', id).maybeSingle();
  ...
  Future<Review> updateReview({
    ...
    final data = await _client.from('reviews').update(update).eq('id', reviewId).select().single();
    return _fromRow(data);
  }
  ...
  Future<ReviewSummary> getMerchantRatingSummary(String merchantId) async {
    final data = await _client.rpc('get_merchant_rating_summary', params: {'p_merchant_id': merchantId}).maybeSingle();
  ...
  Stream<Review> watchMerchantReviews(String merchantId) {
    return _client.from('reviews').stream(primaryKey: ['id']).eq('merchant_id', merchantId).order('created_at', ascending: false).limit(1).map((rows) => _fromRow(rows.first));
  }
}
