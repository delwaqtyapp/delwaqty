...
> **Last updated:** 2026-09-23 — **ROUND 33 FIX: Hero → controlled 300-360px band (sprint 173)** — User rejected the full-height hero: "home_egypt_hero.png is ONLY a decorative top-section background, approx 30-35% of screen, MUST be a bounded Stack and NEVER control page height." Root cause found: `LayoutBuilder` inside `SliverToBoxAdapter` yields `constraints.maxHeight == infinity` → `inf*0.82 → clamp(560,700)` → hero locked at 700px (~85% of screen). Fix (in `home_page.dart`): dropped LayoutBuilder; `heroH = (MediaQuery.sizeOf(context).height * 0.32).clamp(300.0, 360.0)`; structure = `ClipRRect(vertical bottom 28) → SizedBox(heroH) → Stack(expand)` with SINGLE `Image.asset(home_egypt_hero, cover, gaplessPlayback, errorBuilder=purple gradient)` in Positioned.fill + light lavender overlay (2E146F@.12→transparent→6C3CEB@.12); hero column compacted to fit ~189px fixed + Spacer (legal: bounded context): logo 62→42, wordmark 24→18, دلوقتي 20→14, greeting 26→24 + subtitle 13→12, location pill 44→40, search height 60→54 radius 28; `_HeroOrderCard` REMOVED from inside hero → moved to its own SliverToBoxAdapter right after hero (same card untouched, 104px/radius 26/gradient). Verified: `rg -c home_egypt_hero` == 1 (single use), no leftover `onOrderTap`/LayoutBuilder in hero, no Expanded in unbounded context. Gates: analyze 0, tests 937/937, APK `releases/delwaqty_1.0.0+1_debug_20260923_0600.apk` installed + clean relaunch (pid 20808).
