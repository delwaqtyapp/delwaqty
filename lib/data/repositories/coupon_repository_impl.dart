...
class CouponRepositoryImpl implements CouponRepository {
  ...
  Future<Coupon> applyCoupon(String code, double orderTotal) async {
    try {
      ...
      if (coupon == null) throw ServerException(message: 'Invalid or expired coupon');
  ...
  Future<List<Coupon>> getMerchantCoupons(String merchantId) async {
    try { return await _dataSource.getMerchantCoupons(merchantId); }
    catch (e) { throw ServerException(message: e.toString()); }
  }

  @override
  Future<List<Coupon>> getBranchCoupons(String branchId) async {
    try { return await _dataSource.getBranchCoupons(branchId); }
    catch (e) { throw ServerException(message: e.toString()); }
  }

  @override
  Future<List<Coupon>> getProductCoupons(String productId) async {
    try { return await _dataSource.getProductCoupons(productId); }
    catch (e) { throw ServerException(message: e.toString()); }
  }

  @override
  Future<List<Coupon>> getCategoryCoupons(String categoryId) async {
    try { return await _dataSource.getCategoryCoupons(categoryId); }
    catch (e) { throw ServerException(message: e.toString()); }
  }
