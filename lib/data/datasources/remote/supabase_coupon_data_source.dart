...
final supabaseCouponDataSourceProvider = Provider<SupabaseCouponDataSource>((ref) {
...
class SupabaseCouponDataSource {
  ...
  Coupon _fromRow(Map<String, dynamic> row) => Coupon(
    ...
    minimumOrder: row['minimum_order'] != null ? (row['minimum_order'] as num).toDouble() : null,
  ...
  Future<List<Coupon>> getAvailableCoupons() async {
    final data = await _client.from('coupons').select().eq('is_active', true).order('created_at', ascending: false);
    return (data as List).map((r) => _fromRow(r as Map<String, dynamic>)).where((c) => _deriveStatus(c) == CouponStatus.active).toList();
  }

  Future<Coupon?> getCouponByCode(String code) async {
    final data = await _client.from('coupons').select().eq('code', code.toUpperCase()).maybeSingle();
  ...
  Future<Coupon?> validateCoupon(String code, double orderTotal) async {
    ...
    if (coupon.minimumOrder != null && orderTotal < coupon.minimumOrder!) return null;
    return coupon;
  }

  Future<void> incrementUsage(String code) async {
    await _client.rpc('increment_coupon_usage', params: {'coupon_code': code}).catchError((_) async {
      final coupon = await getCouponByCode(code);
      if (coupon != null && coupon.usedCount != null) {
        await _client.from('coupons').update({'used_count': coupon.usedCount! + 1}).eq('id', coupon.id);
      }
    });
  }

  Future<List<Coupon>> getMerchantCoupons(String merchantId) async {
    final data = await _client.from('coupons').select().eq('merchant_id', merchantId).order('created_at', ascending: false);
  ...
  Future<List<Coupon>> getBranchCoupons(String branchId) async {
    final data = await _client.from('coupons').select().eq('branch_id', branchId).order('created_at', ascending: false);
    return (data as List).map((r) => _fromRow(r as Map<String, dynamic>)).toList();
  }

  Future<List<Coupon>> getProductCoupons(String productId) async {
    final data = await _client.from('coupons').select().eq('product_id', productId).order('created_at', ascending: false);
    return (data as List).map((r) => _fromRow(r as Map<String, dynamic>)).toList();
  }

  Future<List<Coupon>> getCategoryCoupons(String categoryId) async {
    final data = await _client.from('coupons').select().eq('category_id', categoryId).order('created_at', ascending: false);
    return (data as List).map((r) => _fromRow(r as Map<String, dynamic>)).toList();
  }
