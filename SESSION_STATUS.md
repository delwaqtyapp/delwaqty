...
> **Last updated:** 2026-09-23 — **ROUND 33 FINAL FIX: edge-to-edge hero, zero overflows, RTL-safe (sprint 173)** — User's FINAL FIX spec (do NOT change assets; fix implementation only). FOUND ROOT CAUSES: (a) `body: SafeArea(...)` wrapped the ENTIRE HomePage body → hero started below the status bar = white strip above hero; (b) hero brand "Delwa" was navy (spec: white); (c) logo was 42px (spec: 60-75); (d) top row placed notification on start-side/Egypt on end-side (spec: Egypt top-right, notification opposite, no collision); (e) heroH clamp was 300-360 (spec: 360-430); (f) greeting subtitle maxLines 1 (spec: wrap naturally). FIXES (home_page.dart only, assets untouched): (1) removed outer SafeArea → hero starts at very top of screen; wrapped Scaffold in `AnnotatedRegion<SystemUiOverlayStyle>` (status bar transparent + light icons + nav bar transparent); inner hero SafeArea kept (protects content only, background stays edge-to-edge). (2) heroH = `(screenH * 0.38).clamp(360, 430)`. (3) `_buildTopRow`: Egypt identity block FIRST (start side; RTL → right ✓) + wrapped in `Flexible` with `CrossAxisAlignment.start`, tagline maxLines 1 + ellipsis; notification circle second (end side) — zero collision either direction. (4) logo width 42→68 + subtle circular glow only (no fill/border/white box). (5) "Delwa" → white; "Qty" gradient kept; "دلوقتي" plain RTL text, NO flip/mirror anywhere (verified no Transform.flip/Matrix4.diagonal3Values in file). (6) greeting subtitle maxLines 1→2 (wraps), greeting line 1 + ellipsis. (7) location pill untouched (Expanded + ellipsis already correct). Verified: analyze 0, tests 937/937, `rg -c home_egypt_hero` == 1 (single use), only ONE SafeArea left in file (hero content), single heroH line, APK `releases/delwaqty_1.0.0+1_debug_20260923_0700.apk` installed + clean relaunch (pid 7411, zero RenderFlex/overflow logs). Visual checks still user-side (agent cannot see rendered screen): hero behind status bar, icons readable, دلوقتي not mirrored.
