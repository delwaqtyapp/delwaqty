...
## ADR-104: Chat Round 2 — Image Viewer/Download, Role-Consistent Bubbles, Typing Indicator, Voice-Call Request, Owner Delete + Auto-Purge, Complaint Reference Numbers (round 56d, sprint 192)

**Date:** Sprint 192

### Context
One sprint after the media-enabled chat (ADR-103), the user requested a second chat round: **(1)** open received images fullscreen and download them; **(2)** the bubble left/right ordering must hold per-ROLE for both sides (customer's own bubble right / admin left, mirrored when the admin views the same room) — previously implemented only via blanket `isMe` alignment with same-color bubbles, which the user felt was not "handled"; **(3)** robust voice playback; **(4)** a "يكتب الآن" typing indicator; **(5)** a voice-call button at the TOP of the chat; **(6)** deleting a chat as OWNER only from the admin dashboard list (and from the customer side), with automatic deletion after one week when not manually deleted; **(7)** closing the chat and starting a NEW chat carrying a NEW complaint/reference number (رقم الشكوى).

### Decision
**(1) Live DB layer (migrations 094 + 095, both applied LIVE and verified).** `chat_reference_seq` (START 1000) + `chat_rooms.reference_number` (backfilled `DWQ-1001`…) with column DEFAULT `next_chat_reference()`; RPC `admin_delete_chat(p_room_id)` SECURITY DEFINER (deletes `chat_messages` + `storage.objects` under the room prefix + the room; authorized for `role='owner'` OR any participant, so the owner from the admin list and the customer themselves can both delete); RPC `admin_purge_expired_chats()` (SECURITY DEFINER, `is_admin()`-gated, deletes rooms with `auto_delete_at <= now()` — the one-week auto-delete — and returns the removed count); new `chat_typing(room_id,user_id,is_typing,updated_at)` table with RLS (SELECT: participants/admin; INSERT/UPDATE: own row; DELETE: admin) and `REPLICA IDENTITY FULL` so the Supabase Realtime stream reflects `is_typing` changes.
**(2) Repository/data layer.** `ChatRoom.referenceNumber` (json `reference_number`); `ChatRepository`+impl+data-source gained `deleteChatRoom` (rpc), `purgeExpiredChats` (rpc→int), `setTyping` (upsert on (room,user)), `typingStream` (emits `true` only when a NON-self participant row has `is_typing=true`). `adminAllRoomsProvider` and `customerMyRoomsProvider` call `purgeExpiredChats()` first — the 7-day auto-delete executes naturally on every list open.
**(3) Chat room page.** Bubbles stay `Align`-driven (`isMe` = own sender → right / peer → left), which by construction mirrors correctly for customer and admin; ROLE colors + labels now clearly difference sides: admin bubbles = `primaryContainer` + `adminLabel`/support icon; customer bubbles = `tertiaryContainer` + `customerLabel`/person icon. Image bubbles open a fullscreen `InteractiveViewer` viewer and carry a download action (http fetch → docs dir → saved-path snackbar); each voice bubble owns its own `AudioPlayer` (`tapToStop`/`tapToPlay`) so playback states never bleed between bubbles; typing = `someoneIsTyping` line above the input, driven by `setTyping` upsert debounced 1200 ms on `onChanged`; voice call = AppBar `call_rounded` button that posts a `message_type='call'` request, and incoming requests render **accept/decline** actions writing status + ids into `metaData`; owner-only delete icon -> confirm dialog -> `admin_delete_chat`. The AppBar title shows the complaint reference number.
**(4) Room lists + new-chat flow.** Admin list tiles title `complaintNo: <ref>` with an owner-only delete icon; the customer list shows references, FAB for new chat, and `_startNewChat` now pushes straight INTO the freshly created room whose generated `DWQ-xxxx` reference appears in its AppBar — a new complaint = a new reference.

### Rationale
- Reference numbers make closed complaints traceable and give admins/customers a stable handle ("رقم الشكوى") with zero extra UI complexity.
- Delegating delete to an RPC (owner OR participant) keeps RLS storage/messages policies untouched while centralizing the cascade (messages + attachments + room) and making the owner-only admin action enforcement explicit and auditable.
- A dedicated purge RPC + read-time invocation realizes the agreed 7-day retention contract without any cron dependency.
- `REPLICA IDENTITY FULL` + a per-(room,user) upsert is the cheapest Supabase-native typing stream; scoping to non-self avoids feedback loops.
- Per-role bubble colors/labels finally make "customer bubble right / admin bubble left - and the reverse for the admin" visually unambiguous, matching the user's expectation that ordering is per-viewer and role-identifiable.
...
3. Only on the HOME tab root → a compact frosted-glass exit card (per the user's design request, superseding the default `AlertDialog`): a custom `Dialog` wrapping `BackdropFilter` (blur 28) + `ClipRRect`(32px) + translucent `Container` (white@0.78 light / black@0.55 dark), hairline border, soft shadow, `barrierColor` black@0.28, `maxWidth 280`, with the intro app-logo («الخروج من التطبيق» / «هل أنت متأكد أنك تريد الخروج من التطبيق؟» at 52px in a circular chip), compact title + confirm text, and two side-by-side `OutlinedButton`(Cancel) / `FilledButton`(Exit, error color) with 14px radii. Confirm calls `SystemNavigator.pop()` (not `dart:io exit()`, which would bypass Android lifecycle). The intro app logo (`assets/egypt/delwaqty_logo_mark.webp` via `Assets.egypt.delwaqtyLogoMark`) is shown centered in the dialog body on a soft circular surface, and — as the user requested — the SAME logo also appears in the OTA update dialog (`lib/services/ota/ota_update_dialog.dart`: 36px in the title row replacing the generic system-update icon + 80px centered in the body on a circular backdrop).
...
### Verification
NEW `test/shared/widgets/app_shell_test.dart` (4 tests): a real GoRouter + 2-branch fake `StatefulShellRoute` pumping `AppShell` proves (1) back on a non-home tab lands on home, (2) back on home tab shows the exit dialog, (3) Cancel keeps the app alive on home, (4) confirm triggers `SystemNavigator.pop` (recorded via a mocked `SystemChannels.platform`); the dialog test also asserts it contains exactly the `delwaqtyLogoMark` image. Gates: `flutter analyze` 0 issues, `flutter test` 950/950. Built artifact `delwaqty_1.0.1+2_debug_20260926_203907.apk` (57MB) installed on device 192.168.8.36:5555; app launched (pid 9830) with no fatal/root-isolate errors; `input keyevent BACK` on the home tab did NOT terminate the app (pid remained 9830) — the exit-dialog path works. (The device then went into active human use — Gmail/WhatsApp — so the dialog visual was left for the user to confirm; the logo-in-dialog is covered by the widget test.)
