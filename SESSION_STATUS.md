...
> **Last updated:** 2026-09-23 — **ROUND 30: «الأكثر طلبا» v2 — full-bleed HERO cards (image fills card, service data overlaid, zero flat colors)** — User pushed for an even stronger modern look: image ON the card (not a side thumb), data integrated in the card, no plain colors. Full redesign of `_MostRequestedCard` (commerce_discovery_page.dart only) to a hero card: Stack(fit: expand) of 3 layers — (1) full-bleed cover image (cacheWidth dpr×560, error → type-gradient + emoji `_ImageFallback`), (2) NON-flat scrim LinearGradient topCenter→bottomCenter [typeColor@.30 → transparent → black@.45 → black@.82] (color-tinted top, legible dark base), (3) overlays: top row = glass type chip (black@.35 bg + white@.25 border, emoji+label) + Spacer + green «مفتوح» capsule + 24px verified circle; bottom block over dark base = white w800 18px name (1 line ellipsis) + glass rating chip (star+score±count) + glass delivery chip (icon+time) + glass fee chip (Wrap). Glass = Colors.white.withValues(alpha:.18) + white borders (no Blur — ADR-safe). Added subtle cover-flow depth in the carousel itemBuilder via AnimatedScale `1 - distance.clamp*0.07` (120ms easeOut, RTL-safe); removed the now-unused `_ServiceChip`; RepaintBoundary kept. All text maxLines1+ellipsis, no Flexible in unbounded flex. Gates: analyze **0 issues**, tests **937/937** (3 regression tests green), APK `releases/delwaqty_1.0.0+1_debug_20260923_0225.apk` installed + relaunched clean (pid 11941). Committed sprint 170.
