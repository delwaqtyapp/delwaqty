...
> **Last updated:** 2026-09-23 — **ROUND 33 ULTIMATE FIX: menu drawer + Cairo دلوقتي + zero overflow (sprint 173)** — User's FINAL HERO FIX: (1) RESTORE side menu button — added `_MenuCircleButton` (40px glass circle, `AppIcons.navDrawer`, white .2/border .35/shadow) FIRST in top row (RTL: right) + notification last (RTL: left); opens a NEW consumer `Drawer` added to the shell Scaffold (app_shell.dart) built from the existing dormant infrastructure `FeatureRegistry.allDrawerEntries` (home/profile/notifications/safety/audio-log entries with badge streams via `StreamProvider.autoDispose.family` — Riverpod 3 sealed Ref forced this deviation; header logo 52 + DelwaQty, body + settings sections). Menu opens via shell `scaffoldKey` (HomePage has its own inner Scaffold so `Scaffold.of(context)` would hit the wrong one — justified deviation). (2) Egypt identity moved OUT of top row into its own compact block BELOW it (gold مصر w900 24 + white tagline 13 w600, start-aligned → right in RTL) — hierarchy: Menu/Notification → مصر → logo → دلوقتي → greeting → tagline → pill → search. (3) ZERO overflow: heroH = `(h*0.40).clamp(370,430)`; fixed children sum ≈291 (topRow 40, egypt ~40, lockup 60-68/wordmark16/دلوقتي14, pill 38, search 50) + `Flexible` wrapping the greeting (absorbs any deficit — mathematically no overflow even at min heroH 370 with status inset 48: 291+22≤322); `SafeArea(top:true, bottom:false)` inside hero only. (4) «دلوقتي» NOW matches the Intro exactly (the bug was: our purple appNameAr vs intro's Cairo w600 `#F7F7FA` letterSpacing 0.5 in `Directionality(rtl)`) — applied identical treatment, fontSize 14 ≈ 45-55% of 68px logo width, centered under wordmark, no mirror/scaleX(-1). (5) Assets untouched. Verified: analyze 0, tests 937/937, `rg -c home_egypt_hero` = 1, single SafeArea (hero), no flip transforms, APK `releases/delwaqty_1.0.0+1_debug_20260923_0800.apk` installed + clean relaunch (pid 300). Visual/user checks remain: menu button visible+clickable, drawer opens, image complete behind status bar, دلوقتي not mirrored.
