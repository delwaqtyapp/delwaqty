...
final supabaseInventoryDataSourceProvider = Provider<SupabaseInventoryDataSource>((ref) {
...
class SupabaseInventoryDataSource {
  ...
  Future<ProductInventory?> getInventory(String productId) async {
    final data = await _client.from('product_inventory').select().eq('product_id', productId).maybeSingle();
    return data != null ? _fromRow(data) : null;
  }

  Future<List<ProductInventory>> getMerchantInventory(String merchantId) async {
    final data = await _client.from('product_inventory').select().eq('merchant_id', merchantId).order('updated_at', ascending: false);
  ...
  Future<ProductInventory> upsertInventory({
    ...
    final data = await _client.from('product_inventory').upsert({
  ...
  Future<ProductInventory> adjustStock({
    ...
    if (existing == null) throw Exception('Inventory not found for product: $productId');
  ...
  Future<ProductInventory> releaseStock({
    ...
    final existing = await getInventory(productId);
    if (existing == null) throw Exception('Inventory not found for product: $productId');
    final newReserved = (existing.reservedQuantity - quantity).clamp(0, existing.reservedQuantity);
    return upsertInventory(
      productId: productId,
      merchantId: existing.merchantId,
      stockQuantity: existing.stockQuantity,
  ...
  Future<List<String>> getOutOfStockProductIds(String merchantId) async {
    final data = await _client.from('product_inventory').select('product_id').eq('merchant_id', merchantId).eq('is_in_stock', false);
    return (data as List).map((r) => r['product_id'] as String).toList();
  }

  Future<List<String>> getLowStockProductIds(String merchantId) async {
    final data = await _client.from('product_inventory').select('product_id, stock_quantity, low_stock_threshold').eq('merchant_id', merchantId);
    return (data as List)
        .where((r) => (r['stock_quantity'] as int) <= (r['low_stock_threshold'] as int) && (r['stock_quantity'] as int) > 0)
        .map((r) => r['product_id'] as String)
        .toList();
  }

  Stream<ProductInventory> watchInventory(String productId) {
    return _client.from('product_inventory').stream(primaryKey: ['id']).eq('product_id', productId).limit(1).map((rows) => _fromRow(rows.first));
  }
}
