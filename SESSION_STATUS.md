...
> **Last updated:** 2026-09-23 — **ROUND 32: filter row under search — «الكل» label hidden + unify with home-page tab style** — User: the word «الكل» in the filter box under search is hidden, and pressing any filter should give the same color/style as the bottom-of-list buttons on the home page (`_DiscoveryTabs` pills: selected = brand gradient [brandPurpleDeep→brandViolet] + white w700; unselected = surfaceContainerHighest@.35 + outlineVariant@.15 border + onSurfaceVariant w600; PressableScale press animation + AnimatedContainer 200ms). Root cause of hidden label: the «الكل» used a BARE M3 `FilterChip` (default theme) whose selected-state label theming rendered invisible on this app theme. Fix: (1) replaced the raw FilterChip in `commerce_discovery_page.dart` with the same custom pill (text-only, exactly like home tabs); (2) rewrote `MerchantTypeChip` (merchant_type_chip.dart) from FilterChip to the identical gradient-pill style, keeping its type icon (white when selected, onSurfaceVariant otherwise) + label via AppTextStyles.labelLarge, onTap toggles `onSelected(!selected)`; (3) updated the chip tests (renders PressableScale pill / callback fires / works without onSelected — was asserting FilterChip). Gates: analyze **0 issues**, tests **937/937**, APK `releases/delwaqty_1.0.0+1_debug_20260923_0410.apk` installed + relaunched clean (pid 8933). Committed sprint 172.
