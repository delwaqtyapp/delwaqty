...
final supabaseBranchDataSourceProvider = Provider<SupabaseBranchDataSource>((ref) {
...
class SupabaseBranchDataSource {
  ...
  Future<List<Branch>> getBranches(String merchantId) async {
    final data = await _client.from('branches').select().eq('merchant_id', merchantId).order('is_primary', ascending: false);
    return (data as List).map((r) => _fromRow(r as Map<String, dynamic>)).toList();
  }

  Future<Branch?> getBranchById(String id) async {
    final data = await _client.from('branches').select().eq('id', id).maybeSingle();
  ...
  Future<Branch> updateBranch(Branch branch) async {
    final data = await _client.from('branches').update({
      'name': branch.name,
      'address': branch.address,
      'latitude': branch.latitude,
      'longitude': branch.longitude,
