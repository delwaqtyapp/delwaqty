...
> **Last updated:** 2026-09-24 — **ROUND 46: GREETING WORD ORDER FIXED — «أهلاً» FIRST, THEN THE USER NAME (RTL paragraph) (sprint 181f)** — User: «الترتيب الاول فى رساله الترحيب على الصوره الاول بنقول اهلا وبعيدين اسم المستخدم انت عكستها ف صلحها». Root cause: the greeting Text renders inside the hero's LTR-wrapped row (ADR-089), so the greeting paragraph resolved LTR and bidi could place the user name to the RIGHT of «أهلاً» — reading right-to-left the name appeared FIRST (e.g. visually «المستخدم، أهلاً»). The logical string was already «أهلاً، $name` (welcome-key first). FIX: each `_UserGreeting` line is now wrapped in its own `Directionality(textDirection: TextDirection.rtl)` with `textAlign: TextAlign.right` — the paragraph is explicitly RTL, so «أهلاً» is ALWAYS the first (rightmost) word and the name follows to its left, regardless of whether the name is Arabic or Latin. Layout unchanged (row-level LTR wrapper still pins search-left/greeting-right; Column `crossAxisAlignment.end` evaluated under that LTR = visually right). Gates: `flutter analyze` **0 issues**, `flutter test` **936/936** (6 responsive incl. notch inset-58 — no overflow/exception), APK `releases/delwaqty_1.0.0+1_debug_20260924_0000.apk` installed + relaunched clean (pid 14183, **logcat NO RenderFlex/FATAL**), screencap `releases/screenshot_hero_0000.png` (~5MB full-frame). DOCUMENTED in ADR-092.

> **Last updated:** 2026-09-24 — **ROUND 45: GREETING FLUSH TO THE FAR RIGHT EDGE + SMALL, UNTRUNCATED SUBTITLE (sprint 180f)** — User: «خلى الكلام يبدء من اقصى يمين الصوره للترحيب والكلام تحته يكون صغير وظاهر ومفيش جمله مقطوعه». (1) Removed the `ConstrainedBox(200)` that pinned the greeting box to a fixed width and made it start mid-image — the greeting now rides the `Flexible`+`Align(topRight)` only, so Arabic text begins AT the far right edge of the image. (2) `_UserGreeting` re-aligned to `crossAxisAlignment: end` + `TextAlign.end` on both lines → text hugs the right edge. (3) Subtitle («دائماً معك ... كل احتياجاتك في مكان واحد») is now smaller (`(sw*0.030).clamp(11,13)` vs 12-14) and **wraps up to 2 lines instead of being cut** (`maxLines: 2` both lines, title too) — no truncated sentence for long names/screens. (4) `contentCoreH` second-row estimate raised 60→88dp so the wrapped greeting can never overflow the hero. Gates: `flutter analyze` **0 issues**, `flutter test` **936/936** (6 responsive incl. notch inset-58 — no overflow/exception), APK `releases/delwaqty_1.0.0+1_debug_20260923_2300.apk` installed + relaunched clean (pid 10105, **logcat NO RenderFlex/FATAL**), screencap `releases/screenshot_hero_2300.png` (~5MB full-frame). DOCUMENTED in ADR-091.

> **Last updated:** 2026-09-24 — **ROUND 44: EGYPT STATEMENT REMOVED, PER-USER GREETING IN ITS PLACE (sprint 179f)** — User: «شيل كلمه مصر بتقدم دايما للعالم من تخت زر القائمه ومكانها حط الترحيب لكل مستخدم». (1) `_EgyptStatement` widget + its `egyptStatementTitle/Tagline` usage DELETED from the hero (the Egypt slogan is gone entirely from the home page top). (2) NEW `_UserGreeting` widget in the same place (right side of the image, under the side-menu button): title = `ل10n.welcome + '، ' + name` → «أهلاً، محمد» for an authenticated user / just «أهلاً» for guest; subtitle = the app's unused `greetingSubtitle` key («دائماً معك ... كل احتياجاتك في مكان واحد»). Greeting name comes from `widget.authState is AuthAuthenticated ? user.fullName : null` (name already available in `_EgyptHero`, no new provider). Kept the same style (gold w900 title + white w600 shadowed subtitle) and the same width/overflow guard (Flexible + Align topRight + maxWidth 200). Gates: `flutter analyze` **0 issues**, `flutter test` **936/936** (6 responsive incl. notch inset-58 — no overflow/exception), APK `releases/delwaqty_1.0.0+1_debug_20260923_2200.apk` installed + relaunched clean (pid 29668, **logcat NO RenderFlex/FATAL**), screencap `releases/screenshot_hero_2200.png` (~5MB full-frame). DOCUMENTED in ADR-090.

> **Last updated:** 2026-09-24 — **ROUND 43: SEARCH CIRCLE ON THE LEFT (BETWEEN NOTIFICATION & LOCATION) + STATEMENT ON THE RIGHT — RTL FIX (sprint 178f)** — User: «حرك زر البحث الجديد على الصورة من مكانه فى اليمين الى اليسار بين اللوكيشن و الاشعارات وضع كلمه مصر بتقدم دايما للعالم اسفل زر القائمه الجانبيه فى اليمين». Root cause: the hero's second row had NO explicit text direction, so the ambient RTL (Arabic app) reversed the spaceBetween Row — the search circle (first child) rendered on the RIGHT and the statement on the LEFT, the OPPOSITE of the requested layout. FIX: wrapped the row in `Directionality(textDirection: TextDirection.ltr)` (same pattern as the top-bar row which hosts notification-left / menu-right), so **search circle renders LEFT under the notification button** (band between notification and the bottom-left location badge) and **«مصر دائما بتقدم للعالم» renders RIGHT under the side-menu button**. Statement is still width-bounded (Flexible + Align topRight + maxWidth 200 → safe ellipsis, no overflow). Gates: `flutter analyze` **0 issues**, `flutter test` **936/936** (6 responsive incl. notch inset-58 — no overflow/exception), APK `releases/delwaqty_1.0.0+1_debug_20260923_2100.apk` installed + relaunched clean (pid 14958, **logcat NO RenderFlex/FATAL**), screencap `releases/screenshot_hero_2100.png` (~5MB full-frame). DOCUMENTED in ADR-089.

> **Last updated:** 2026-09-24 — **ROUND 42: EGYPT STATEMENT BACK ON THE IMAGE (UNDER MENU) + SEARCH AS SMALL CIRCLE (UNDER NOTIFICATION) (sprint 177f)** — User: «ارفع كلمه مصر دائما بتقدم للعالم اسفل زر القائمه الجانبيه وشريط البحث صغرة ل علامه بحث صغيره بحجم زر الاشعارات وخليها تبقى تحت زر الاشعارات». (1) **Egypt statement returned INSIDE the hero image** — `_EgyptStatement` (gold title + white tagline w600 + soft shadow, right side of the image, directly below the side-menu circle) replaces the big glass search bar; its old standalone sliver ABOVE the direct-order button **removed** (no duplicate). (2) **Search became a small circular glass button** — NEW `_SearchCircleButton` (same `topButtonSize`/`topIconSize` as the notification/menu circles: white 0.2 + 0.35 border + soft shadow, `AppIcons.actionSearch`, tap → `context.push('/search')`) sitting BELOW the notification button (left), mirroring the button row above. The old full-width `_buildHeroSearch` method deleted. (3) **Layout overflow hardening**: the statement now lives in a `Row(crossAxisAlignment: start, mainAxisAlignment: spaceBetween)` with `Flexible`+`Align(topRight)`+`ConstrainedBox(maxWidth: 200)` — round 41 leftover risk gone: a long locale string can ONLY ellipsize, never overflow (the old plain Spacer layout threw «a RenderFlex overflowed by 92 pixels» in the EN test locale) → responsive suite still clean. (4) Location badge `maxWidth` 178→150 so it can't kiss the statement's left edge at small widths. Hero formula unchanged (`heroH = max(heroImageH, contentH)`, contentCoreH = topButtonSize + zoneGap + 60). Gates: `flutter analyze` **0 issues**, `flutter test` **936/936** (6 responsive incl. notch 366×800 inset 58 — no overflow/exception), APK `releases/delwaqty_1.0.0+1_debug_20260923_2000.apk` installed + relaunched clean (pid 29263, **logcat NO RenderFlex/FATAL**), screencap `releases/screenshot_hero_2000.png` (~5MB full-frame). DOCUMENTED in ADR-088.

> **Last updated:** 2026-09-23 — **ROUND 41: FIX HERO BOTTOM OVERFLOW (21px) ON REAL DEVICE (sprint 176f)** — User: «تحت مكان اللوكيشن فى الصورة هتلاقى شريط اصفر فى اسود ظهر على الصورة من الاسفل مكتوب عليه Bottom overload By21 pixels صلح الخطء ده». ROOT CAUSE: device = 1280×2800 @ 3.5 (logical 366×800), status-bar inset ≈ 58dp; the hero's overlay Column (top bar + search + Egypt statement) needed more than `heroImageH (174) − inset − padding`, so the fixed-height flex overflowed ~21-30px. FIX: (1) hero height is now `heroH = max(heroImageH, contentH)` where `contentH = MediaQuery.paddingOf(context).top + 2 + topButtonSize + zoneGap + searchHeight + zoneGap` → overflow is structurally impossible on ANY device; (2) components compressed to fit the image even with big insets: topButton `(sw*0.095).clamp(36,44)`, search `(sw*0.080).clamp(30,38)`, zoneGap `(sh*0.008).clamp(4,9)`; (3) **Egypt statement moved OUT of the image** (it was the content that could never fit) into its own sliver right ABOVE the direct-order button — new `_EgyptStatement` widget (gold title + muted-purple `AppColors.primaryLight` tagline for the light page background), `_buildEgyptStatement` method deleted; (4) location badge bottom tracks `(heroH − heroImageH) + 12` so it stays pinned to the image bottom edge; (5) **NEW TEST**: `home_page_responsive_test.dart` now includes a notch-device case (366×800, top inset 58) that reproduces the exact overflow condition and asserts zero exceptions. Gates: `flutter analyze` **0 issues**, `flutter test` **936/936** (6 responsive cases incl. inset 58), APK `releases/delwaqty_1.0.0+1_debug_20260923_1900.apk` installed + relaunched clean (pid 28485, **logcat has NO RenderFlex/overflow**), screencap `releases/screenshot_hero_1900.png`. DOCUMENTED in ADR-087.

> **Last updated:** 2026-09-23 — **ROUND 40: BUTTON UNDER IMAGE EDGE + CATEGORIES FIRST (sprint 175f)** — User: «ارفع اطلب طلبك مباشرة تحت حدود الصورة مباشرة من الاسفل وشيل الفريم البنفسجى من تحت الزر وارفع تحت زر اطلب طلبك مباشرة الفئات الرئيسيه وما يليها». Restructured the home top: (1) **Hero == image height only** — `_EgyptHero` now `SizedBox(height: heroImageH)`; the column inside it = top bar + glass search + Egypt statement only (the old floating `_DirectOrderButton` + trailing `Spacer` moved OUT); the hero's bottom purple tint (`0xFF6C3CEB` 0.12 band) removed from the overlay gradient → the "purple frame under the button" is GONE; (2) **`_DirectOrderButton` is now its own sliver** right below the hero (Padding 20/10/20/0) — sits DIRECTLY under the image's bottom edge, animated motorcycle + wind untouched; (3) **Categories first**: sliver order is now hero → button → `_CompactCategories` → `_PromoCarousel` → discovery → footer (the user wants الفئات الرئيسية مباشرة تحت الزر، والعروض بعدها). Gates: `flutter analyze` **0 issues**, `flutter test` **935/935** (incl. 5-size responsive — no overflow at 360x640…800x1280), APK `releases/delwaqty_1.0.0+1_debug_20260923_1800.apk` installed + relaunched clean (pid 26353, no FATAL/overflow), real screencap `releases/screenshot_hero_1800.png`. DOCUMENTED in ADR-086.

> **Last updated:** 2026-09-23 — **ROUND 39: MOTORCYCLE SPEED WIND + REMOVED MID-PAGE ORDER CARD (sprint 174f)** — User: «انا عايز موتسيكل دليفرى وكان فيه هوا وراه من السرعه والزر بتاع اطلب طلب مباشر الى انت عملته حلو امسح الزر الى تحته وشيل الفريم الى تحت الزرار الجديد وارفع باقى التطبيق تحت الزر عشان ميبقاش فيه فراغ». (1) **Speed wind** behind the hero motorcycle in `_DirectOrderButton`: 4 white rounded wind bars stretched OPPOSITE the travel direction (`slide - wind*(12+8*i)`, wind = cos(πt) → bars vanish at turnarounds where speed = 0, physically correct), fading opacity (0.55→0.13) + tapered widths + soft glow; (2) **Deleted the mid-page `_HeroOrderCard`** («الزر الى تحته» + «الفريم») — sliver+class fully removed; (3) **`_PromoCarousel` now sits directly under the hero** — no gap, app content raised. The NEW `_DirectOrderButton` (in-hero, animated, gradient, `/direct-delivery` CTA) untouched. Gates: `flutter analyze` **0 issues**, `flutter test` **935/935** (incl. 5-size responsive), APK `releases/delwaqty_1.0.0+1_debug_20260923_1700.apk` installed + relaunched clean (pid 14777, no FATAL/overflow), real screencap `releases/screenshot_hero_1700.png` (~5MB full-frame). DOCUMENTED in ADR-085.

> **Last updated:** 2026-09-23 — **ROUND 38: HERO REDESIGN — MINI LOGO TOP, INTRO ARABIC NAME, GLASS SEARCH, LOCATION BADGE, ANIMATED MOTORCYCLE CTA (sprint 173f)** — User: «ارفعلى الان اللوجو بشكل صغير جدا فوق فى الصورة وتحه كلمه DelwaQty بالانجليزى امسحها وخد الصح الى فى الانترو وصغرها وحطها بشكل صغير تحت اللوجو وبعد كده الغى المربع الى فوق اطلب طلبك مباشرة وارفع البحث خليه زر تحت الاشعارات بشكل زجاجى واللوكيشن حطه فى اسفل الصورة من اليسار فى مربع صغير زجاجى وارفع بعدها زر اطلب طلبك مباشر فى اول التطبيق تحت الصورة مباشرة واجعل فيه شكل موتسيكل دليفرى يتحرك باسلوب انميشن احترافى». Full hero rebuild in `home_page.dart` `_EgyptHero`: (1) **Mini lockup in the top bar** — logo shrunk to 44dp + intro-style Arabic «دلوقتي» (Cairo w600, 12dp, near-white) under it, CENTERED in the top row between the notification/menu circles (English "DelwaQty" wordmark DELETED — `_buildBrandLockup` removed); (2) **Greeting box removed** (`_buildGreeting`+`_greeting` deleted); (3) **Search raised** — glass button directly under the top row (`_buildHeroSearch` kept, same location-pill style); (4) **Location moved** — small glass badge (28dp, maxWidth 178, bottom-left) OVERLAID at the bottom-left of the hero IMAGE inside the natural-fit image box (`_buildLocationBadge`, old `_buildLocationPill` removed); (5) **NEW `_DirectOrderButton`** — gradient CTA (60dp, radius 30, brandPurple→brandBlue, glow) placed right under the image («في اول التطبيق تحت الصورة مباشرة»), containing an ANIMATED delivery MOTORCYCLE (`Icons.two_wheeler_rounded`): 2.4s looping `AnimationController` + sin() motion — glide ±16px with ease + subtle vertical bob + two fading trail ghosts behind (professional effect); text = `orderDirectly` + `fastestWayToOrder`, arrow chevron; tap → `context.push('/direct-delivery')` (same as the mid-page order card); (6) Egypt statement kept small right-aligned. Cleanup: `_buildTopControlsRow` replaced by inline LTR Row [notif, Spacer, miniLockup, Spacer, menu]; `dart:math` import added. Gates: `flutter analyze` **0 issues**, `flutter test` **935/935** (incl. 5-size responsive suite), APK `releases/delwaqty_1.0.0+1_debug_20260923_1600.apk` installed + relaunched clean (pid 23462, no FATAL/overflow), real screencap `releases/screenshot_hero_1600.png`. DOCUMENTED in ADR-084.

> **Last updated:** 2026-09-23 — **ROUND 37: HERO RESIZED — NATURAL IMAGE + SMALLER TOP BAR/LOCATION + GLASS SEARCH (sprint 173e)** — User: «صورة البار العلوى انت مكبرها اكبر من حجمها ومكبر كمان اللوكيشن اكبر من حجمه وصندوق البحث عايزه باستايل اللوكيشن الشفاف والجميل والصورة الى تحتهم صغرها على قد شاشه كل تليفون ومتحركهاش من فوق صغر من الاجناب ولو من تحت كمان مفيش مشكله اذا احتجت وفوق ثابت خلى التطبيق يتكيف على طل هاتف تلقائيا». Root causes found: (1) the hero background `home_egypt_hero.png` (1821x864) was rendered `BoxFit.cover` inside a 430×~470 hero → scaled to fill height, zoom-cropping ~57% of width (looks «مكبّره»); (2) `topButtonSize=(sw*0.13).clamp(48,56)` + `locationHeight=(sw*0.11).clamp(44,48)` too big; (3) search used `PremiumSearchField` (solid white .85 capsule) while location used the glass pill; (4) hero formula `(sh*0.48).clamp(430,560)` oversized. Fixes in `home_page.dart` `_EgyptHero`: **image now natural-fit** — top-anchored `Positioned` box `height: screenWidth*(864/1821)` + `BoxFit.fitWidth` + `alignment: topCenter` (never upscaled; bottom sits on the lavender gradient; «فوق ثابت»); **topBar circles 45** logical `(sw*0.105).clamp(40,48)`; **location pill 34** `(sw*0.08).clamp(32,38)`; **hero search rebuilt INLINE as the exact location glass style** (white .18 + white .3 border + radius 22 + soft shadow, search icon + hint + circular filter, `onTap → context.go('/search')`) — `PremiumSearchField` import dropped (shared widget untouched); hero `(sh*0.46).clamp(400,540)` — app auto-adapts to any phone («يتكيف تلقائيا»). Also fixed a mid-edit `Widget Widget` signature slip + leftover unused import. Gates: `flutter analyze` **0 issues**, `flutter test` **935/935** (incl. 5-size responsive suite), APK `releases/delwaqty_1.0.0+1_debug_20260923_1500.apk` installed + relaunched clean (pid 3015, no FATAL/overflow), real screencap `releases/screenshot_hero_1500.png` (1.3MB, device capture works again). DOCUMENTED in ADR-083.

> **Last updated:** 2026-09-23 — **ROUND 36: SIDE MENU ENRICHED — FULL OLD GLASS MENU CONTENT (sprint 173d)** — User: «بس القائمه الجانبيه ناقصها حاجات كتير… لسه ناقصها حاجات كانت في القائمه الزجاجيه القديمه». Forensic comparison (old `_DrawerPanel` a82e652 vs current — IDENTICAL, both filter body+footer; only 3 modules ever registered `DrawerEntry` at a82e652 AND today) → the menu LOOKED empty because only home/notifications/profile entries existed. Fix = register the app's REAL customer screens into the glass drawer (registry list, order = exact drawer sequence), so the menu is full like the old branded menu: **الرئيسية (/home)، الخدمات (/services)، الإشعارات (badge)، الملف الشخصي (/profile)، طلباتي (/orders)، محفظتي (/wallet)، توصيلة (/direct-delivery)، الشكاوي (/my-complaints)، المكافآت (/rewards)**. (1) `drawerEntries` added to: `home_module.dart` (+2nd entry `services`→`/services` icon grid_view), `delivery_module.dart` (`direct-delivery`→`/direct-delivery` icon moped), `orders_module.dart` (receipt_long→`/orders`), `wallet_module.dart` (account_balance_wallet→`/wallet`), `complaints_module.dart` (warning_amber→`/my-complaints`), `rewards_module.dart` (card_giftcard→`/rewards`); all labels use existing module-name l10n keys (0 new keys needed — `allServices` verified in en+ar); `module_registry.dart` reordered so registerAll order = drawer order (home, notifications, profile, orders, wallet, direct-delivery, complaints, rewards, then entry-less modules). (2) `_DrawerPanel` in `app_shell.dart`: header now FIXED at top; entries + dark-mode/language toggles + footer + logout wrapped in `Expanded(SingleChildScrollView(Column))` — guarantees no clipping/red-stripe on small screens with 9 entries (the likely «ناقصها» culprit). Glass look untouched (blur 32/width 280/topEnd+bottomEnd 28). (3) Duplicate `drawerEntries` getter in `orders_module.dart` removed; unused riverpod imports in `delivery_module.dart` removed; `const DirectDeliveryPage()`. (4) NEW test `test/shared/generated_side_menu_test.dart`: registry `allDrawerEntries` contains exactly the 9 ids (and NOT search). Gates: `flutter analyze` **0 issues**, `flutter test` **935/935** (934+1), APK `releases/delwaqty_1.0.0+1_debug_20260923_1400.apk` (223MB) built + installed + relaunched clean (pid 13475, logcat clean — no FATAL/overflow). NOTE: device `screencap` was failing this session (50-byte files, «Failed to take screenshot» adb/device glitch; foreground verified via dumpsys = com.delwaqty.app). DOCUMENTED in ADR-082.

> **Last updated:** 2026-09-23 — **ROUND 35: COMPLETE REMOVAL OF RIDE-SAFETY + VOICE-RECORDING REMNANTS, SINGLE MENU (sprint 173c)** — User: «انت عملت اتنين قايمه جانبيه ومحطتش القايمه الجانبيه اللي كانت قبل الصوره… وضيفت في الكود امان خاص برحلات اوبر واحنا لغينا الكود ده نهائيا… ولا عايز تسجيل الصوت. شيل كل ده من المنصه تماما». (1) **Duplicate menu fixed**: the sprint-35 shell AppBar (leading menu_rounded + title + notifications) had been re-added with the drawer restore → removed entirely → shell back to round-33 shape (body + floating dock only), single drawer trigger = hero menu circle via `AppShell.scaffoldKey.currentState?.openDrawer()`. (2) **Deleted permanently — ride-safety suite**: `lib/features/customer/safety/` (SafetyModule — safety-hub/trusted-contacts/safety-settings drawer entries), `lib/features/customer/service_audio_logs/` (ServiceAudioLogsModule — the audio-log drawer entry + `AudioRecordingDialog`), module registrations removed from `lib/customer/module_registry.dart` + `lib/provider/module_registry.dart` + `lib/driver/module_registry.dart`; `_AudioRecordingCard`+`AudioRecordingDialog` block removed from `order_tracking_page.dart` (incl. now-dead isActive/userId locals + auth imports); admin `serviceAudioLogs` sidebar item (`floating_sidebar_overlay.dart`) + its `/service-audio-logs` nav removed; admin `AdminEmergencyPage` (SOS monitor) file + route + import deleted; realtime channels `sos-alerts`/`trusted-contacts` removed (`realtime_channel_constants.dart`); **46 ride-safety/audio l10n keys** removed from en+ar (2005→1959 each) via python json (validated). (3) **Kept intentionally** `features/customer/ride/` + `features/customer/driver/` — the delivery feature depends on their infra (ride.dart/ride_map/ride_providers used by `delivery_tracking_page` + `supabase_delivery_data_source`); consistent with ADR-080 dormant classification. DOCUMENTED in ADR-081 (user-ordered removal, §12.1). Gates: `flutter analyze` **0 issues**, `flutter test` **934/934** (−8 safety tests), APK `releases/delwaqty_1.0.0+1_debug_20260923_1300.apk` installed + relaunched clean (pid 26144, no FATAL/overflow), drawer opened via adb tap + screencap `releases/screenshot_menu_hist_1300.png`.

> **Last updated:** 2026-09-23 — **ROUND 34: ORIGINAL GLASS SIDE MENU RESTORED (sprint 173b)** — User: «القائمه الجانبيه انت بدلتها بواحده تانيه تماما، كان فيه قائمه جانبيه الاول شفافه وشكلها حلو، رجعها مكان الحالية». Restored the sprint-35 approved glass drawer design (commit `a82e652`: "glass drawer with slide animation") in place of the solid drawer built during the sprint-173 hero saga. Ported `_DrawerPanel` faithfully into `lib/shared/widgets/app_shell.dart`: `Drawer(backgroundColor: transparent, elevation: 0)` hosting the panel = `_DrawerPanel` (Align centerStart) → `ClipRRect` (topEnd/bottomEnd 28) → `BackdropFilter(ImageFilter.blur(32,32))` → `Container(width: 280, maxHeight 85%, glass: white .82 light / white .08 dark, 0.5 border, shadow blur 40 offset(8,0))` → SafeArea → Column: user header (48 gradient circle initial + name fullName??username + email), body drawer entries, dark-mode tile + Switch (`themeModeProvider.notifier.toggleTheme`), language tile + subtitle (`localeProvider.notifier.toggleLocale`), footer entries, destructive logout tile + confirm AlertDialog (`authStateProvider.notifier.signOut`). `_openDrawer` → keeps `AppShell.scaffoldKey.currentState?.openDrawer()` contract so `home_page.dart` menu button is UNTOUCHED. Framework Drawer slides from the side per RTL automatically (right in Arabic). NOTE — ADR-044 blur exception: BackdropFilter restored BY USER DIRECTIVE as an approved legacy design (not new code). Lint pass: info `sort_constructors_first` (constructor hoisted above static field), `use_decorated_box` (Container→DecoratedBox @shell glass dock), `use_null_aware_elements` (`if (trailing != null) trailing!` → `?trailing`) → analyze **0**. Gates: `flutter analyze` **0 issues**, `flutter test` **942/942** (incl. 5-size responsive suite), APK `releases/delwaqty_1.0.0+1_debug_20260923_1200.apk` installed + relaunched clean (pid 28174, no FATAL/overflow), drawer opened via adb tap at computed menu-button coords (x≈1056, y≈172 — TopControls is LTR-forced: notification left / menu right), screencap `releases/screenshot_menu_hist_1200.png` (263KB = dimmed scrim + glass panel frame, vs 5MB full home). Committed sprint 173b / pushed.

> **Last updated:** 2026-09-22 — **ROUND 24: fix «اكتشف بالقرب منك» empty (NULL user_id crash)** — User reported ALL discovery lists hidden. Root cause (verified against live DB): `service_providers` has 44 rows, 41 are NULL-`user_id` seeds (migration made the column nullable in ROUND 13), but `ServiceProvider.fromJson` (`service_provider.g.dart:12`) did `json['user_id'] as String` → `TypeError: type 'Null' is not a subtype of type 'String'` on EVERY seed row → `discoveryEntriesProvider` (calls `getProviders()`) → AsyncError → the whole «اكتشف بالقرب منك` section (and every providers page) rendered nothing. Fix: `userId: json['user_id'] as String? ?? ''` (hand-edit .g.dart — established repo pattern, build_runner unavailable) + regression test `ServiceProvider.fromJson tolerates NULL user_id like seeded rows` in `test/features/home_services/snake_case_api_entities_test.dart`. Merchants (28 rows) + providers (44 rows) verified clean via anon REST (only missing columns are handled: rating_count→total_reviews, image_url→logo_url); car_products seller_id non-null (no latent bug). Gate: `flutter analyze` 0 issues, `flutter test` **934/934** (933 + 1 new regression test), APK `releases/delwaqty_1.0.0+1_debug_20260922_1905.apk` installed + relaunched clean (pid 29113, no FATAL). Committed sprint 164.

> **Last updated:** 2026-09-22 — **ROUND 25: All-Merchants image-failure crash — errorBuilder baked into every remaining Image.network** — User reported some merchant squares in «جميع المتاجر» show «HTTP request failed, statusCode: 400» breaking the tile (Al-Baik's Wikimedia SVG-as-.png URL returns 400), and asked to fix every error of this type permanently. Root cause: `MerchantCard.presentation/widgets/merchant_card.dart:175` used `Image.network(merchant.imageUrl!)` with NO `errorBuilder` → an image failure rendered a raw error box inside the card, wrecking its layout. Full-suite scan (per-file count of Image.network vs errorBuilder across lib/features/customer + lib/shared) found exactly 3 files with Image.network but no errorBuilder; the other 11 already had it. Fixed: (1) `merchant_card.dart` — new local `_imagePlaceholder(context, colorScheme)` (same gradient + `_typeIcon` as the null branch) now used by BOTH null and error; `_typeLabel` got `maxLines: 1` + ellipsis (name row + delivery row already safe). (2) `restaurant_menu_page.dart` — product-thumb errorBuilder → fastfood icon fallback (same as null branch). (3) `restaurant_detail_page.dart` — header errorBuilder → new `_headerImagePlaceholder(theme)` (gradient + restaurant icon, same as null branch). Any failed image now renders the styled placeholder instantly — no crash box, no broken tile, ever. Gate: `flutter analyze` **0 issues**, `flutter test` **934/934**, APK `releases/delwaqty_1.0.0+1_debug_20260922_1925.apk` installed + relaunched clean (pid 17753, no FATAL). Committed sprint 165.

> **Last updated:** 2026-09-22 — **ROUND 26: /market card bottom-overflow (58px) + filter chips no-selection — fixed permanently** — User: in «جميع المتاجر» the featured cards showed a yellow strip «Bottom overflowed by 58 pixels» under the merchant text, and the category buttons under the search bar had hidden labels with NO selected state on tap. Root causes: (1) The featured carousel wrapped `MerchantCard` (width 260 → natural height = 260×9/16 image ≈146 + text ≈92 = 238) inside a fixed `SizedBox(height: 180)` → exact 58px overflow; (2) `MerchantTypeChip` was an `ActionChip` with NO selected support → tapping updated the provider but nothing visually selected; labels clipped inside the 40px chip row. Fixed: (1) removed the fixed height from the featured carousel SizedBox (horizontal ListView shrink-wraps to the card's natural height — scales correctly on any device/font scale); grid `childAspectRatio` 0.8 → 0.74 in BOTH data + shimmer grids (taller cells); `MerchantCard` bottom text block wrapped in `Flexible` (permanent no-overflow guarantee in any constrained parent). (2) `MerchantTypeChip` → controlled `FilterChip` with `selected`/`onSelected`/`showCheckmark` + selectedColor/side, feed `selected: selectedType == type` from the page; chip row height 40 → 48 so long Arabic labels are never clipped; tests updated for the new API (933→ same count 934 all pass). Gate: `flutter analyze` **0 issues**, `flutter test` **934/934**, APK `releases/delwaqty_1.0.0+1_debug_20260922_1945.apk` installed + relaunched clean (pid 8288, no FATAL/overflowed). Committed sprint 166.

> **Last updated:** 2026-09-23 — **ROUND 27: /market white-screen crash after scroll (Flexible-in-unbounded) + image decode memory — FIXED with regression tests** — User: after our ROUND 26 build, scrolling down/up on «جميع المتاجر» suddenly showed a WHITE SCREEN and the app crashed out to the launcher completely. Forensic evidence on device: no Dart exception logged (no E/flutter), `ActivityManager Killing ... isFromBinderDied:true ... remove task` at 00:50:57 + HONOR SWAP freeze (score:999, reclaimRatio:80) + an ANR file right before («freezing 12479»). Diagnosis: (A) ROUND 26 wrapped MerchantCard's bottom text in `Flexible` — inside the featured carousel (horizontal ListView → UNBOUNDED height in the outer vertical ListView) a non-zero-flex child with unbounded incoming constraints throws the hard RenderFlex error «children have non-zero flex but incoming height constraints are unbounded» (white screen + process death on a plain debug APK); (B) /market decodes every network image at FULL native resolution (picsum 3000×2000 ≈ 24MB bitmap each, ~38 images/page) → jank/ANR/OOM under memory pressure. Fixed: (1) REVERTED `Flexible` → plain `Padding` in `merchant_card.dart` (safe in bounded AND unbounded hosts; the natural-height carousel + grid 0.74 already guarantee correct sizing); added `mainAxisSize` implied by structure. (2) Added `cacheWidth: (devicePixelRatio * 340).round()` to MerchantCard's image and `* 200` to `_MostRequestedCard`'s — images now decode at display resolution (~1/10 the memory, spin-free scrolling). (3) NEW regression tests `test/features/commerce/presentation/merchant_card_unbounded_test.dart`: MerchantCard pumps inside a `SingleChildScrollView` + width-260 card (the EXACT featured-carousel context — would throw before the fix) + inside a 0.74 grid cell. Gate: `flutter analyze` **0 issues**, `flutter test` **936/936** (934+2), APK `releases/delwaqty_1.0.0+1_debug_20260923_0105.apk` installed + relaunched clean (pid 26745). Committed sprint 167.

> **Last updated:** 2026-09-23 — **ROUND 33 RESPONSIVE ENGINEERING (sprint 173)** — User: hero must be FULLY responsive (not built for one phone size); use MediaQuery.sizeOf + paddingOf + LayoutBuilder; all element sizes from screen-derived formulas with clamps; NO hard-coded top/left Positioned; content capped at 600 width on tablet; landscape reduces hero; genuinely adapts on any new device. Implemented in home_page.dart `_EgyptHero`: `size/padding/isPortrait` read from MediaQuery; `heroH = isPortrait ? (sh*0.48).clamp(430,560) : (sh*0.60).clamp(300,420)` (~48-53% portrait on phones, ~44% tablet, ≤420 landscape); all sizes responsive: `horizontalPadding=(sw*0.055).clamp(16,28)`, `topButtonSize=(sw*0.13).clamp(48,56)`, `logoSize=(sw*0.20).clamp(72,92)`, `wordmarkSize`, `arabicSize=(logoSize*0.19).clamp(13,17)`, `egyptTitleSize=(sw*0.062).clamp(24,28)`, `egyptTaglineSize`, `welcomeTitleSize`, `welcomeSubtitleSize=(sw*0.038).clamp(14,17)`, `locationHeight=(sw*0.11).clamp(44,48)`, `searchHeight=(sw*0.13).clamp(52,56)`, `zoneGap=(sh*0.014).clamp(6,14)`; button circles = topButtonSize + topIconSize. Structure: Stack(expand)[DecoratedBox background via `DecorationImage(AssetImage hero, cover, center)` (fills hero — height never from image) + lavender overlay + SafeArea(top:true) + Center(ConstrainedBox(maxWidth:600)) + Padding(horizontalPadding) + LayoutBuilder + Column: TopControls(LTR-forced: notification left/menu right) → zoneGap → Egypt(topRight) → Spacer(2) → Lockup(logo+wordmark+دلوقتي intro-exact RTL) → Spacer(3) → Flexible(Greeting — absorbs deficit at small sizes) → Spacer(2) → Location → zoneGap → Search]. NEW acceptance via real widget tests: `test/features/customer/home/home_page_responsive_test.dart` pumps HomePage at **360×640, 360×800, 412×915, 430×932, 800×1280** with provider fakes + FlutterError trap — **ALL FIVE PASS with ZERO overflow/exception** (this is the pixel-less way to verify responsive engineering). Lint fix: renamed underscore locals in test. Full gates: analyze **0**, tests **942/942** (937+5), APK `releases/delwaqty_1.0.0+1_debug_20260923_1100.apk` installed + clean relaunch (pid 7478), screencap `releases/screenshot_hero_1100.png` saved for user's eyes.

> **Last updated:** 2026-09-22 — **ROUND 22: single moving strip (marquee) with ALL services + حذف الشريط السفلي** — User request: the buttons UNDER the top categories strip (the whole `_ServicesSection`) should live in «عرض الكل» only, and the home page should keep ONE moving strip that shows all the app's services (commerce categories + every booking service) with a «عرض الكل» tile at the END; when the customer doesn't tap it, the strip keeps auto-scrolling slowly, endlessly (aesthetic marquee). Implemented in `home_page.dart` only: (1) new `_InfiniteStrip` StatefulWidget (Ticker + SingleTickerProviderStateMixin, NOT Timer — test-safe): two identical copies of the tiles (each copy ends with a 12px separator), `_cycleWidth` measured post-frame via GlobalKey with 3 retries (`_scheduleMeasure`), `_onTick` advances offset at const 40px/s (dt clamped at 0.25s, `offset %= _cycleWidth` via jumpTo → seamless loop), pauses on pointer down/up/cancel via Listener, respects `MediaQuery.disableAnimationsOf` (single copy + AlwaysScrollableScrollPhysics for manual scroll + ticker stopped), guards `mounted`/`_scroll.hasClients`, clamps offset when the list shrinks; (2) `_CompactCategories` now watches BOTH `activeCategoriesProvider` + `_homeServiceCategoriesProvider` and renders items = ranked categories + ALL services (priority now leads doctor/nurse/teacher/barber again) + `_TopStripShowAll()` always at the end → `/services`; loading = shimmer, error/empty = `_buildStrip([..._topBookingItems, const _TopStripShowAll()])`; `_buildServiceTile` routes `deliveryCar` → `/home-services/cars`; (3) `_ServicesSection` class + sliver + `_movedToTop` + pharmacy/visibleCount/showAllTile logic deleted entirely. Reviewer (general fallback — gemini reviewer quota-blocked again) found C1 CRITICAL (ticker double-started in initState+didChangeDependencies → debug assert crash) + M1 (reduce-motion unreachable content) + N1/N2: all fixed via ticker create-only-in-initState + guarded start/stop in didChangeDependencies, conditional physics, offset clamp on shrink, measure retry. Gate (re-verified independently by QA): `flutter analyze` **0 issues**, `flutter test` **933/933**, QA **APPROVE**, APK built debug/customer (223MB) → `releases/delwaqty_1.0.0+1_debug_20260922_1755.apk`. Committed sprint 162. **Follow-up (sprint 163, user request):** strip now manually draggable with the finger (forward/backward) — `physics: AlwaysScrollableScrollPhysics()` in ALL modes, and auto-scroll pauses during drag/fling via `_scroll.position.isScrollingNotifier.value` guard (isScrolling is deprecated in 3.47) then RESUMES automatically from where the user released, seamlessly. Gate: `flutter analyze` 0 issues, `flutter test` 933/933, APK `releases/delwaqty_1.0.0+1_debug_20260922_1830.apk` installed + relaunched clean (no FATAL). Committed sprint 163.

> **Last updated:** 2026-09-22 — **ROUND 21: booking-service buttons merged into the top categories strip** — User request: in the customer app home page, the four booking-service buttons (حجز دكتور / ممرض / مدرسين / حجز حلاق) currently in the lower `_ServicesSection` strip (under the basic services list) should be integrated into the TOP `_CompactCategories` strip — directly after the pharmacy (صيدلية) tile and before the "عرض الكل" tile — and stay with the top buttons (removed from the lower section so they don't duplicate). Implemented in `home_page.dart` (only file): added top-level `_serviceLabel/_serviceIcon/_serviceColor` (moved from `_ServicesSection`, no dupes), sealed `_TopStripItem` hierarchy (Category/Service/ShowAll) + `_buildStrip` helper; `_CompactCategories` now builds items = visible categories → pharmacy → 4 booking service tiles → view-all. Reviewer fixes applied: MAJOR-1 pharmacy-aware slice (pharmacy ranks 6th but `visibleCount=6` was cutting it out → slice now extends to include pharmacy so services always anchor right after it), MAJOR-2 `pharmacyFound` dedupe guard, MAJOR-3 error/empty provider branches now render `_buildStrip(_topBookingItems)` so booking entry points survive outages, MINOR-1 single source `_topBookingTypes`/`_movedToTop`, MINOR-3 maxLines 2→1, MINOR-4 comment removed. `_ServicesSection` filters `_movedToTop` (no duplication) and skips when empty; `_homeServiceCategoriesProvider` priority no longer leads with doctor/nurse/teacher/barber. Completed pre-existing half-finished `merchant.g.dart` working-tree edit: added missing `_$MerchantTypeFromJson` tolerant helper (`firstWhere/orElse→other`, mirrors ROUND-18 service_booking.g.dart pattern; build_runner impossible on this device so hand-edited .g.dart is the established repo pattern). Gate: `flutter analyze` **0 issues**, `flutter test` **933/933**, QA **APPROVE**, APK built via `flutter build apk --debug --flavor customer -t lib/customer/main.dart --dart-define-from-file=.env.dev` (Success). INTRO untouched.
