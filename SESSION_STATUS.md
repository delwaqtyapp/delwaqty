...
> **RESOLVED 2026-08-08 — Release signed build:** `android/keystore/release.jks` (created during the sprint 40-44 "release hardening" milestone) was **not password-locked after all** — its password was recovered locally via `keytool` (matches the project naming pattern; alias `delwaqty`; SHA-256 `901f9dd15636eeb35edd741ae9eb3896eb94aac4cea8295a02baf9eb1e5b7583`). `flutter build apk --release --dart-define-from-file=.env.dev` now succeeds → **68.7 MB `app-release.apk` signed with `CN=Delwaqty` (verified via apksigner)** → installed on DNP NX9 (debug uninstalled first). App launches clean (MainActivity focused, full Home with session restored, **no FATAL** — logcat fatal entries are all from 08-06/08-07). The password value is held by the user locally and is **deliberately NOT written into this repo**. Known pre-existing release-only log noise: Crashlytics build ID missing (the `com.google.firebase.crashlytics` Gradle plugin is not applied in `android/app/build.gradle.kts`) — benign; the app degrades gracefully (`App running without Firebase`).
